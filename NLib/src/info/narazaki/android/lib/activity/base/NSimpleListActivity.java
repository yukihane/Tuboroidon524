...
abstract public class NSimpleListActivity extends NListActivity {
    ...
    protected void hasInitialData(final boolean has_initial_data) {
    ...
    protected void saveResumePosition() {
        ...
        if (resume_data_ == null && list_adapter_.getCount() > 0 && has_initial_data_) {
            final PositionData pos_data = getCurrentItemPos();
            if (pos_data != null)
                resume_data_ = pos_data;
        }
    }

    public PositionData getCurrentItemPos() {
        if (list_adapter_ == null || list_adapter_.getCount() == 0)
            return null;
        final View target = getListView().getChildAt(0);
        if (target == null)
            return null;
        return new PositionData(getListView().getFirstVisiblePosition(), target.getTop());
    }

    protected void setResumeItemPos(final int resume_item_pos, final int resume_item_top_y) {
    ...
    protected void onSaveResumePositon(final PositionData stat) {
    }
    ...
    protected final void setReloadInProgress(final boolean reload_in_progress) {
    ...
    protected void resumeItemPos(final Runnable callback) {
        if (resume_data_ != null && list_adapter_ != null) {
            // setListPositionFromTop(resume_data_.position_, resume_data_.y_,
            // callback);
