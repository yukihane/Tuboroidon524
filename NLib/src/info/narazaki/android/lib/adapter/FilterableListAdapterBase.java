...
SimpleListAdapterBase<T_DATA> {
    ...
    public FilterableListAdapterBase(final Activity activity) {
        super();
        activity_ = activity;
    ...
    public void setFilter(final Filter<T_DATA> filter, final Runnable callback) {
        filter_ = filter;
    ...
    public void setFilter(final PrepareFilter<T_DATA> prepare, final Filter<T_DATA> filter, final Runnable callback) {
        filter_ = filter;
    ...
    public void setComparer(final Comparator<T_DATA> comparator, final Runnable callback) {
        comparator_ = comparator;
        applyFilter(callback);
    }

    public int getMappedPosition(final int global_pos) {
        final ArrayList<Integer> mapped_pos_list = mapped_pos_list_;
        if (mapped_pos_list == null)
            return global_pos;
        if (mapped_pos_list.size() <= global_pos || global_pos < 0)
            return -1;
                ...
                activity_.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (filter_ == null || filter_.filter(data)) {
                            ...
                            if (filter_ != null)
                                mapped_pos_list_.add(mapped_pos_list_.size());

                        } else {
                            if (filter_ != null)
                                mapped_pos_list_.add(-1);
                        }
                    }
                });
            }
    ...
    private void addDataOrderd(final ArrayList<T_DATA> list, final T_DATA data) {
        if (comparator_ == null) {
            ...
            final int index = Collections.binarySearch(list, data, comparator_);
            if (index >= 0) {
        ...
        postAdapterThread(new Runnable() {
            ...
            public void run() {
                ...
                if (filter_ == null) {
                    ...
                    int i = 0;
                    for (final T_DATA data : inner_data_list_) {
    ...
    public void setDataList(final List<T_DATA> data_list, final Runnable callback) {
        addAndApplyFilter(data_list, true, callback);
    }

    @Override
    public void setDataList(final ArrayList<T_DATA> data_list) {
        setDataList(data_list, null);
    }

    public T_DATA getInnerData(final int position) {
        try {
