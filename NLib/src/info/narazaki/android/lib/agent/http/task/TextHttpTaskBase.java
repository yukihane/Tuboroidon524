...
public abstract class TextHttpTaskBase extends HttpTaskBase {
    ...
    public TextHttpTaskBase(final String request_uri) {
        super(request_uri);
    }
    ...
    protected void dispatchHttpResponse(final HttpResponse res, final InputStream res_is) throws InterruptedException, IOException {
    ...
    @Override
    protected final boolean sendRequest(final String request_uri) throws InterruptedException, ClientProtocolException,
    IOException {
        if (Thread.interrupted())
            throw new InterruptedException();
        final HttpRequestBase req = createHttpRequest(request_uri);
        if (req == null)
            throw new ClientProtocolException();
        if (!setRequestParameters(req))
            throw new ClientProtocolException();
