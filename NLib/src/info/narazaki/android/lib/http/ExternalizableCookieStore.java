...
public class ExternalizableCookieStore extends BasicCookieStore implements Externalizable {
    ...
    public void readExternal(final ObjectInput input) throws IOException, ClassNotFoundException {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (input.readBoolean())
                cookie.setComment(input.readUTF());
            if (input.readBoolean())
                cookie.setExpiryDate(new Date(input.readLong()));

            if (input.readBoolean())
                cookie.setDomain(input.readUTF());
            if (input.readBoolean())
    ...
    public void writeExternal(final ObjectOutput output) throws IOException {
        String data;
        Date date;
        final List<Cookie> cookie_list = getCookies();
        output.writeInt(cookie_list.size());
        for (final Cookie cookie : cookie_list) {
