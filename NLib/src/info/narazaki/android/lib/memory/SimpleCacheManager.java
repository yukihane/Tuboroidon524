...
public class SimpleCacheManager<T_TAG, T_DATA> {
    ...
    public SimpleCacheManager(final int min_cache_size, final int max_cache_size) {
        ...
        if (min_cache_size_ > 0) {
            hard_cache_ = new LinkedHashMap<T_TAG, T_DATA>(min_cache_size_ / 2, 0.75f, true) {
                ...
                @Override
                protected boolean removeEldestEntry(final LinkedHashMap.Entry<T_TAG, T_DATA> eldest) {
        ...
        soft_cache_ = new LinkedHashMap<T_TAG, SoftReference<T_DATA>>(max_cache_size_ / 2, 0.75f, true) {
            ...
            @Override
            protected boolean removeEldestEntry(final LinkedHashMap.Entry<T_TAG, SoftReference<T_DATA>> eldest) {
    ...
    public synchronized T_DATA getData(final T_TAG tag, final boolean update_order) {
        final SoftReference<T_DATA> ref = soft_cache_.get(tag);
        if (ref == null)
            return null;
        final T_DATA data = ref.get();
        if (data == null)
            return null;

        if (hard_cache_ != null)
            hard_cache_.put(tag, data);

        return data;
    }

    public synchronized void setData(final T_TAG tag, final T_DATA data) {
        if (hard_cache_ != null)
            hard_cache_.put(tag, data);
        soft_cache_.put(tag, new SoftReference<T_DATA>(data));
    }

    public synchronized void removeData(final T_TAG tag) {
        if (hard_cache_ != null)
            hard_cache_.remove(tag);
