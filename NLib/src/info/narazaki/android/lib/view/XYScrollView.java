...
public class XYScrollView extends FrameLayout {
    ...
    public XYScrollView(final Context context) {
        this(context, null);
    }

    public XYScrollView(final Context context, final AttributeSet attrs) {
        super(context, attrs);
        initScrollView();
        setFillViewport(false);
    }

    public XYScrollView(final Context context, final AttributeSet attrs, final int defStyle) {
    ...
    private boolean inChild(final int x, final int y) {
        if (getChildCount() > 0) {
    ...
    public boolean onTouchEvent(final MotionEvent ev) {
        ...
        case MotionEvent.ACTION_UP:
            if (mIsBeingDragged) {
                ...
                final int initialVelocityX = (int) velocityTracker.getXVelocity();
    ...
    private View findFocusableViewInMyBounds(final boolean leftFocus, final int left, final boolean topFocus,
            final int top, final View preferredFocusable) {
        /*
    ...
    private View findFocusableViewInBounds(final boolean leftFocus, final int left, final int right, final boolean topFocus, final int top, final int bottom) {
    ...
    private boolean scrollAndFocus(final int direction, final int left, final int right, final int top, final int bottom) {
        ...
        final boolean goLeft = direction == View.FOCUS_LEFT;

        final int height = getHeight();
        final int containerTop = getScrollY();
        final int containerBottom = containerTop + height;
        final boolean goUp = direction == View.FOCUS_UP;
    ...
    public boolean arrowScroll(final int direction) {
        ...
        if (nextFocused != null && isWithinDeltaOfScreen(nextFocused, maxJumpX, getWidth(), maxJumpY, getHeight())) {
            ...
            final int scrollDeltaX = computeScrollDeltaToGetChildRectOnScreenX(mTempRect);
            final int scrollDeltaY = computeScrollDeltaToGetChildRectOnScreenY(mTempRect);
            ...
            if (direction == View.FOCUS_LEFT && getScrollX() < scrollDeltaX) {
                ...
                final int daRight = getChildAt(0).getRight();

                final int screenRight = getScrollX() + getWidth();
    ...
    private boolean isOffScreen(final View descendant) {
        return !isWithinDeltaOfScreen(descendant, 0, getWidth(), 0, getHeight());
    }

    private boolean isWithinDeltaOfScreen(final View descendant, final int deltaX, final int width, final int deltaY, final int height) {
    ...
    private void doScrollXY(final int deltaX, final int deltaY) {
    ...
    private int computeScrollDeltaToGetChildRectOnScreenX(final Rect rect) {
        if (getChildCount() == 0)
            return 0;
        ...
        if (rect.right > screenRight && rect.left > screenLeft) {
            ...
            final int distanceToRight = right - screenRight;
    ...
    private int computeScrollDeltaToGetChildRectOnScreenY(final Rect rect) {
        if (getChildCount() == 0)
            return 0;
    ...
    public void fling(final int velocityX, final int velocityY) {
        if (getChildCount() > 0) {
            ...
            final int direction = velocityY != 0 ? (movingDown ? View.FOCUS_DOWN : View.FOCUS_UP)
