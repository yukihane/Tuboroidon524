...
public class ThreadEntryListActivity extends SearchableListActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        // リロード時ジャンプ
        final int jump_on_reloaded_num = thread_data_.getJumpEntryNum(thread_uri_);
        ...
        final View footer_row = LayoutInflater.from(this).inflate(R.layout.entry_list_footer_row, null);
                    ...
                    if (doubleTap) {
                        ...
                        if (filter_.type_ == ParcelableFilterData.TYPE_PELATION) {
                            ...
                            Toast.makeText(ThreadEntryListActivity.this,
                                    getString(R.string.ctx_menu_find_related_entries), Toast.LENGTH_SHORT).show();
    ...
    private void showDialogReplyTo(final ThreadEntryData entry_data) {
        final String[] menu_strings = new String[] { getString(R.string.ctx_submenu_reply_to_this_entry),
    ...
    private void showDialogAddIgnore(final ThreadEntryData entry_data) {
        ...
        if (entry_data.canAddNGID()) {
            ...
            final String[] menu_strings = new String[] { getString(R.string.ctx_menu_add_ignore_word_normal),
                    getString(R.string.ctx_menu_add_ignore_word_gone) };
            builder.setItems(menu_strings, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(final DialogInterface dialog, final int which) {
    ...
    private void showDialogAddNGWord(final ThreadEntryData entry_data, final boolean gone) {
        ...
        final LinearLayout layout_view = (LinearLayout) layout_inflater.inflate(R.layout.add_ngword_dialog, null);
    ...
    private void showDialogCopyToClipboard(final ThreadEntryData entry_data) {
        final String[] menu_strings = new String[] { getString(R.string.ctx_submenu_copy_to_clipboard_id),
    ...
    private void showDialogCopyEntryBody(final ThreadEntryData entry_data) {
        ...
        final AlertDialog.Builder builder = new AlertDialog.Builder(this);
        final LayoutInflater layout_inflater = LayoutInflater.from(this);
        final LinearLayout layout_view = (LinearLayout) layout_inflater.inflate(R.layout.copy_entry_body_dialog, null);
        builder.setView(layout_view);
    ...
    public boolean onCreateOptionsMenu(final Menu menu) {
        ...
        compose_item.setOnMenuItemClickListener(new OnMenuItemClickListener() {
            ...
            public boolean onMenuItemClick(final MenuItem item) {
                final Intent intent = new Intent(ThreadEntryListActivity.this, ThreadEntryEditActivity.class);
                intent.setData(Uri.parse(thread_data_.getThreadURI()));
                startActivityForResult(intent, INTENT_ID_SHOW_ENTRY_EDITOR);
                return false;
            }
    ...
    private void applyViewConfig(final ViewConfig view_config) {
        ...
        lv.setDividerHeight(view_config.entry_divider > 0 ? res.getDimensionPixelSize(R.dimen.entryDividerHeight) : 0);
    ...
    protected void createToolbarButtons() {
        ...
        final ImageButton button_thread_list = (ImageButton) findViewById(R.id.button_toolbar_thread_list);
    ...
    private void setFooterView() {
        if (thread_data_ == null || list_adapter_ == null || list_adapter_.getCount() <= 0)
            return;

        if (footer_view_.getVisibility() == View.GONE)
            footer_view_.setVisibility(View.VISIBLE);
        final ImageView button = (ImageView) footer_view_.findViewById(R.id.entry_footer_image_view);
        final TextView entry_footer_header = (TextView) footer_view_.findViewById(R.id.entry_footer_header);
        final TextView entry_footer_body = (TextView) footer_view_.findViewById(R.id.entry_footer_body);
    ...
    private void onCheckUpdateFinished(final Intent intent) {
        if (list_adapter_ == null)
            return;
        favorite_check_update_progress_ = false;
        final int unread_threads = intent.getIntExtra(TuboroidService.CHECK_UPDATE.NUM_UNREAD_THREADS, 0);
        String message = null;
    ...
    public PositionData getMappedPosition(final PositionData orig) {
        ...
        if (list_adapter_ == null)
            return null;
        final int pos = ((ThreadEntryListAdapter) list_adapter_).getMappedPosition(orig.position_);
        if (pos == -1)
            return null;
        data.position_ = pos;
        return data;
     ...
     * しおりを保存
     */
    private void saveResumeEntryNum(final long seved_entry_id, final int saved_y) {
    ...
    private void resumeSavedEntryNum() {
        if (resume_entry_id_ != Long.MAX_VALUE) {
            ...
            final int pos = ((ThreadEntryListAdapter) list_adapter_).getMappedPosition((int) resume_entry_id_ - 1);
            // Log.d(TAG, "resume_entry_id: " + resume_entry_id_ +
            // " y:"+resume_entry_y_);
    ...
    private void onEntryFilterMode(final long seved_entry_id, final int saved_y) {
        footer_view_.setVisibility(View.GONE);
    ...
    private void updateAnchorBar() {
        ...
        final TextView text_view = (TextView) findViewById(R.id.entry_anchor_stack);
        text_view.setTextSize(getTuboroidApplication().view_config_.entry_header_ * 3 / 2);
        final HorizontalScrollView box = (HorizontalScrollView) findViewById(R.id.entry_anchor_stack_box);
        ...
        if (animation) {
            final ScaleAnimation ta = new ScaleAnimation(1, 1, 0, 1, 0, box.getMeasuredHeight());
            ta.setInterpolator(new OvershootInterpolator());
            ta.setDuration(300);
            box.startAnimation(ta);
        }

        final int on_clicked_bgcolor = obtainStyledAttributes(R.styleable.Theme).getColor(
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (!is_active_ || list_adapter_ == null)
                            return;
                        final ThreadEntryListAdapter list_adapter = (ThreadEntryListAdapter) list_adapter_;
    ...
    private void highlightByEntryId(final long entry_id, final boolean center) {

        final int position = ((ThreadEntryListAdapter) list_adapter_).getMappedPosition((int) entry_id - 1);
        final ListViewEx lvx = (ListViewEx) getListView();
    ...
    public ThreadEntryData getEntryData(final long entry_id) {
        ...
        try {
            return ((ThreadEntryListAdapter) list_adapter_).getInnerData((int) entry_id - 1);
