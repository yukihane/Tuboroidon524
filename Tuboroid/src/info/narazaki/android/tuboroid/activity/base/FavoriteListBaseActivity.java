...
abstract public class FavoriteListBaseActivity extends TuboroidListActivity {
    ...
    private void onReceiveCheckedUpdateIntent(final Intent intent) {
        final int max_items = intent.getIntExtra(TuboroidService.CHECK_UPDATE.MAX, 0);
        final int progress1 = intent.getIntExtra(TuboroidService.CHECK_UPDATE.PROGRESS1, 0);
        final int progress2 = intent.getIntExtra(TuboroidService.CHECK_UPDATE.PROGRESS2, 0);
    ...
    public boolean onKeyDown(final int keyCode, final KeyEvent event) {
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            ...
            case KeyEvent.KEYCODE_R: {
                final ImageButton btn = (ImageButton) findViewById(R.id.button_tab_check_update);
                if (btn != null) {
                    btn.performClick();
                }
            }
    ...
    protected void createNoticeFooter() {
        final View notice_layout = LayoutInflater.from(this).inflate(R.layout.favorite_list_notice_row, null);
    ...
    protected void updateToolbarButtonUpdateOnly() {
        final ImageButton button_updated_only = (ImageButton) findViewById(R.id.button_tab_updated_only);
