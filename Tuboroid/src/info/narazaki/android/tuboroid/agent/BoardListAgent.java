...
public class BoardListAgent {
    ...
    private final LinkedList<Runnable> task_queue_;

    public BoardListAgent(final TuboroidAgentManager agent_manager) {
        super();
    ...
    private void execFetchBoardList(final boolean no_cache, final boolean force_reload,
        ...
        final BoardListFetchedCallback callback = new BoardListFetchedCallback() {
            @Override
            public void onBoardListConnectionFailed() {
                if (callback_orig != null)
                    callback_orig.onBoardListConnectionFailed();
                popTask();
            }

            @Override
            public void onBoardListFetched(final ArrayList<String> boardGroups, final ArrayList<ArrayList<BoardData>> boardList,
                    final StatData boardListStat) {
                if (callback_orig != null)
                    callback_orig.onBoardListFetched(boardGroups, boardList, boardListStat);
                popTask();
            }
            ...
            public void onConnectionOffline() {
                if (callback_orig != null)
                    callback_orig.onConnectionOffline();
                popTask();
            }
        };
    ...
    private synchronized void setBoardCache(final ArrayList<String> board_groups, final ArrayList<ArrayList<BoardData>> board_list,
            final HashMap<BoardIdentifier, BoardData> board_map) {
        data_loaded_ = true;
            ...
            public void read(final BufferedReader reader) {
                try {
                    ...
                    while (true) {
                        ...
                        if (line == null)
                            break;
                        final String[] line_splitted = ListUtils.split("\t", line);
                        if (line_splitted.length == 4) {
                            final BoardData data = BoardData.factory(order_id, line_splitted[0], line_splitted[1],
    ...
    private void reloadBBSMenu(final BoardFileFetchedCallback callback) {
        ...
        final SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(agent_manager_.getContext()
                .getApplicationContext());
        final String default_bbsmenu_url = agent_manager_.getContext().getResources()
                .getStringArray(R.array.bbsmenu_url_values)[0];
        final String bbsmenu_url = pref.getString("pref_bbsmenu_url_list", default_bbsmenu_url);

        final HttpGetBoardListTask task = new HttpGetBoardListTask(bbsmenu_url, new HttpGetBoardListTask.Callback() {
            @Override
            ...
            @Override
            public void onBoardListReceived(final List<BoardData> data_list) {
    ...
    private void saveBoardListFile(final List<BoardData> data_list, final BoardFileFetchedCallback callback) {
        agent_manager_.getFileAgent().writeFile(board_list_filepath_, new DataFileAgent.FileWriteUTF8StreamCallback() {
            ...
            public void write(final Writer writer) throws IOException {
                for (final BoardData data : data_list) {
    ...
    private void migrateBoardList(final List<BoardData> data_list, final BoardFileFetchedCallback callback) {
        final ArrayList<BoardData> data_list_temp = new ArrayList<BoardData>(data_list);
    ...
    private void migrateBoardListImpl(final List<BoardData> data_list, final BoardFileFetchedCallback callback) {
        ...
        final BoardData target_data = data_list.remove(0);
        agent_manager_.getDBAgent().migrateBoardData(target_data, new SQLiteAgent.MigrateBoardCallback() {
            ...
            @Override
            public void onMigrated(final BoardData fromData, final BoardData toData) {
                final LinkedList<String> from_dirs = fromData.getLocalDatDir(agent_manager_.getContext());
                final LinkedList<String> to_dirs = toData.getLocalDatDir(agent_manager_.getContext());
                ...
                while (from_dirs.size() != 0) {
                    final String from_dir_temp = from_dirs.removeFirst();
                    final File from_dir_file = new File(from_dir_temp);
                    final String to_dir_temp = to_dirs.removeFirst();
    ...
    private void mergeCachedBoardList(final List<BoardData> data_list, final BoardListFetchedCallback callback) {
        ...
        agent_manager_.getDBAgent().getBoardList(new SQLiteAgentBase.DbResultReceiver() {
            ...
            public void onQuery(final Cursor cursor) {
                ...
                String current_group = null;
                ArrayList<BoardData> current_list = null;
                for (final BoardData data : data_list_db) {
                    final BoardData target = board_map.get(data.server_def_);
                    if (target != null) {
        ...
        agent_manager_.getFileAgent().writeFile(board_list_stat_filepath_,
                new DataFileAgent.FileWriteUTF8StreamCallback() {
                    ...
                    public void write(final Writer writer) throws IOException {
                        ...
                        writer.append('\n');
                        for (final Boolean data : board_list_stat_.expand_stat_list_) {
    ...
    // 取得メソッド
    private BoardData getBoardData(final BoardIdentifier server_def) {
        if (server_def.board_server_.length() == 0 || server_def.board_tag_.length() == 0)
            return null;

        final BoardData data = board_map_.get(server_def);
        if (data != null)
            return data;
    ...
    private void execGetBoardData(final BoardIdentifier server_def, final boolean is_new_external,
        ...
        if (is_new_external)
            board_data.is_external_ = true;

        agent_manager_.getDBAgent().insertBoardData(board_data, callback, new Runnable() {
            @Override
            public void run() {
                final HttpGetBoardDataTask task = board_data.factoryHttpGetBoardDataTask(new HttpGetBoardDataTask.Callback() {
                    @Override
                        ...
                        agent_manager_.getDBAgent().updateBoardData(newBoardData, new DbTransaction() {
                            ...
                            public void run() {
                                execFetchBoardList(true, false, new BoardListFetchedCallback() {
                                    @Override
                                    public void onBoardListFetched(final ArrayList<String> boardGroups,
                                            final ArrayList<ArrayList<BoardData>> boardList,
    ...
    private void execDeleteBoard(final BoardData board_data, final Runnable callback) {
        final LinkedList<String> list = board_data.getLocalDatDir(agent_manager_.getContext());
