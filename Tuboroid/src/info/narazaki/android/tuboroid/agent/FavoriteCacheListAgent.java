...
public class FavoriteCacheListAgent {
    ...
    public FavoriteCacheListAgent(final TuboroidAgentManager agent_manager) {
        super();
    ...
    private synchronized void popTask() {
        ...
        if (task_list_.size() == 0)
            return;
        final Runnable runnable = task_list_.getFirst();
        executor_.submit(runnable);
    }
    ...
    private void processOrderMap(final BufferedReader reader, final FavoriteListFetchedCallback callback) {
        ...
        try {
            while (true) {
                ...
                if (tokens.length == 2) {
                    final String uri = tokens[0];
                    final String sort_order = tokens[1];
    ...
    private void loadBoardDataList(final FavoriteListFetchedCallback callback) {
        agent_manager_.getDBAgent().getFavoriteBoardList(new SQLiteAgentBase.DbResultReceiver() {
            ...
            public void onError() {
                if (callback != null)
                    callback.onFavoriteListFetched(new ArrayList<FavoriteItemData>());
                popTask();
            }
        });
    }
    ...
    private void processNewDataList(final ArrayList<FavoriteItemData> new_item_list,
        ...
        if (callback != null)
            callback.onFavoriteListFetched(new_item_list);
        popTask();
    }

    private void mergeAndSetOrderMap(final ArrayList<FavoriteItemData> new_item_list) {
        for (final FavoriteItemData data : new_item_list) {
            final String uri = data.getURI();
            final Integer order_id = order_map_.get(uri);
            if (order_id != null)
                data.order_id_ = order_id;
        }
        ...
        agent_manager_.getFileAgent().writeFile(
                TuboroidApplication.getInternalStoragePath(agent_manager_.getContext(), FAVORITE_FILE)
                    ...
                    public void write(final Writer writer) throws IOException {
                        for (final Entry<String, Integer> data : order_map_.entrySet()) {
    ...
    private void setDataList(final ArrayList<FavoriteItemData> new_item_list) {
        ...
        for (final FavoriteItemData data : item_list_) {
            final String uri = data.getURI();
    ...
    private void fetchNextFavoriteThreadImpl(final ThreadData thread_data, final ArrayList<FavoriteItemData> data_list,
        ...
        final String thread_uri = thread_data.getThreadURI();
        ThreadData first_unread = null;
        int unread_thread_count = 0;
        for (final FavoriteItemData data : data_list) {
            if (data.isThread()) {
                final ThreadData target = data.getThreadData();
    ...
    private void deleteFavoriteListImpl(final ArrayList<FavoriteItemData> delete_list, final Runnable callback) {
        ...
        for (final FavoriteItemData data : delete_list) {
            if (data.isBoard()) {
                final BoardData board_data = data.getBoardData();
        ...
        final Runnable callback_with_pop = new Runnable() {
            @Override
            public void run() {
                if (callback != null)
                    callback.run();
    ...
    public void delFavorite(final BoardData board_data, final Runnable callback) {
        board_data.is_favorite_ = false;
        agent_manager_.getDBAgent().updateBoardDataFavorite(board_data, null);
        final String uri = board_data.getSubjectsURI();
    ...
    public void delFavorite(final Find2chKeyData key_data, final Runnable callback) {
        ...
        final String uri = key_data.getSearchURI();
        delFavorite(uri, callback);
    }
    ...
    private void delFavoriteImpl(final String uri, final Runnable callback) {
        final FavoriteItemData data = item_map_.remove(uri);
        if (data != null)
    ...
    private void addFavoriteImpl(final FavoriteItemData target, final int rule, final Runnable callback) {
        final String uri = target.getURI();
    ...
    private void addFavoriteThreadFollowBoard(final FavoriteItemData target) {
        ...
        final ThreadData target_data = target.getThreadData();
    ...
    private void addFavoriteSimilar(final FavoriteItemData target) {
        ...
        for (final FavoriteItemData data : item_list_) {
            if (data.isThread()) {
                final ThreadData thread_data = data.getThreadData();
                final int rate = differ.similarity(thread_data.thread_name_, target_data.thread_name_);
                if (rate > LevenshteinDistanceCalc.MAX_SIMILARITY_RATE / 3 && rate >= max) {
                    final boolean same_board = thread_data.isSameBoard(target_data);
                ...
                final BoardData board_data = data.getBoardData();
                final boolean same_board = board_data.isSameBoard(target_data);
                if (!max_is_same_board || same_board) {
                    max = 0;
                    max_index = i;
                    max_is_same_board = same_board;
