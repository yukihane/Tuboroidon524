...
public class Find2chTask {
    ...
    private final HttpTaskAgentInterface multi_http_agent_;
    private boolean is_alive_;

    public Find2chTask(final Context context, final HttpTaskAgentInterface http_agent) {
        super();
        context_ = context;
    ...
    public void searchViaFind2chImpl(final String key, final int page, final int order,
        ...
        final HttpFind2chTask task = new HttpFind2chTask(context_, key, page, order, new HttpFind2chTask.Callback() {
            @Override
            public void onReceived(final ArrayList<Find2chResultData> dataList, final int found_items) {
                executor_.submit(new Runnable() {
                    ...
                    public void run() {
                        if (is_alive_) {
                            if (page == 0)
                                callback.onFirstReceived(found_items);
