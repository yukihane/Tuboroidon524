...
public class ImageFetchAgent {
    ...
    private class ImageLocation {
        ...
        public ImageLocation(final String imageUri, final int maxWidth, final int maxHeight) {
            super();
            image_uri_ = imageUri;
            max_width_ = maxWidth;
        ...
        public boolean equals(final Object o) {
            if (o instanceof ImageLocation) {
                final ImageLocation target = (ImageLocation) o;
                if (!image_uri_.equals(target.image_uri_))
                    return false;
                if (max_width_ != target.max_width_)
                    return false;
                if (max_height_ != target.max_height_)
                    return false;
                return true;
            }
    ...
    private synchronized void sendCallback(final ImageLocation location, final Bitmap bitmap) {
        if (location != null) {
            final LinkedList<BitmapFetchedCallback> list = callbacks_map_.get(location);
            callbacks_map_.remove(location);
            if (list != null) {
                for (final BitmapFetchedCallback callback : list) {
                    if (callback != null) {
                        if (bitmap != null) {
    ...
    private final HashMap<File, Bitmap> bitmap_hard_cache_ = new LinkedHashMap<File, Bitmap>(MAX_BITMAP_HARD_CACHE / 2,
        ...
        @Override
        protected boolean removeEldestEntry(final LinkedHashMap.Entry<File, Bitmap> eldest) {
            if (size() <= MAX_BITMAP_HARD_CACHE)
                return false;
            return true;
        }
    };
    ...
    private void storeBitmapCache(final File file, final Bitmap bitmap) {
        if (bitmap == null)
            return;
    ...
    private void deleteBitmapCache(final File file) {
        synchronized (bitmap_soft_cache_) {
    ...
    private Bitmap getBitmapCache(final File file) {
        synchronized (bitmap_soft_cache_) {
            final SoftReference<Bitmap> ref = bitmap_soft_cache_.get(file);
            if (ref == null)
                return null;
            bitmap_hard_cache_.remove(file);
    ...
    public ImageFetchAgent(final TuboroidAgentManager agent_manager) {
        super();
    ...
    public void deleteImage(final File image_local_file) {
        pushTask(new Runnable() {
            @Override
            public void run() {
                final File parent_dir = image_local_file.getParentFile();
                if (!parent_dir.exists() || !parent_dir.canRead() || !parent_dir.isDirectory())
                    return;

                final NFileNameInfo info = new NFileNameInfo(image_local_file);
                final String file_prefix = info.getFileName();
                final File[] file_list = parent_dir.listFiles(new FilenameFilter() {
                    @Override
    ...
    public synchronized boolean fetchImage(final BitmapFetchedCallback callback, final File image_local_file,
        ...
        final File cache_file = getScaledCacheFile(max_width, max_height, image_local_file);
        final Bitmap bitmap = getBitmapCache(cache_file);
        if (bitmap != null) {
    ...
    private void execFetchImage(final ImageLocation location, final BitmapFetchedCallback callback,
        ...
        final HttpGetFileTask task = new HttpGetFileTask(image_uri, image_local_file, new HttpGetFileTask.Callback() {
            @Override
            public void onFailed() {
                sendCallback(location, null);
            }
    ...
    private Bitmap getBitmap(final int max_width, final int max_height, final File file, final boolean can_cache) {
        ...
        final File cache_file = getScaledCacheFile(max_width, max_height, file);
    ...
    private File getScaledCacheFile(final int max_width, final int max_height, final File file) {
        final NFileNameInfo info = new NFileNameInfo(file);
    ...
    private Bitmap getBulkBitmap(final File file) {
        ...
        final Bitmap bitmap = BitmapFactory.decodeFile(file.getAbsolutePath(), opt);
    ...
    private Bitmap createScaledBitmap(final int max_width, final int max_height, final File file,
        ...
        if (opt.outHeight == -1 || opt.outWidth == -1)
            return null;
        ...
        final int ratio = width_ratio > height_ratio ? width_ratio : height_ratio;
        ...
        if (can_cache) {
            try {
                final File temp_file = File.createTempFile(cache_file.getName(), ".tmp", cache_file.getParentFile());
                final FileOutputStream os = new FileOutputStream(temp_file);
