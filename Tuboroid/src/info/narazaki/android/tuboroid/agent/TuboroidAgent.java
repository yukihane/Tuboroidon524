...
public class TuboroidAgent {
    ...
    private final TuboroidAgentManager man_;

    public TuboroidAgent(final Context context) {
        super();
        context_ = context;
    ...
    public void saveBoardListStat(final NSimpleExpandableListActivity.StatData stat_data, final Runnable callback) {
    ...
    public void updateThreadData(final ThreadData thread_data, final Runnable callback) {
        man_.getDBAgent().updateThreadData(thread_data, new SQLiteAgentBase.DbTransaction() {
            ...
            public void run() {
                if (callback != null)
                    callback.run();
                final Intent intent = new Intent(THREAD_DATA_UPDATED_ACTION);
    ...
    public PostEntryTask.FuturePostEntry postEntry(final ThreadData thread_data, final PostEntryData post_entry_data,
        ...
        final PostEntryTask task = thread_data.factoryPostEntryTask(man_);
    ...
    public CreateNewThreadTask.FutureCreateNewThread createNewThread(final BoardData board_data,
        ...
        final CreateNewThreadTask task = board_data.factoryCreateNewThreadTask(man_);
    ...
    public Future<?> downloadExternalFont(final File local_file, final FontFetchAgent.FontFetchedCallback callback) {
        final FontFetchAgent agent = new FontFetchAgent(man_.getMultiHttpAgent(), callback);
