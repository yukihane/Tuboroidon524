...
public class TuboroidAgentManager {
    ...
    private final String MARU_USER_AGENT = "DOLIB/1.00";
    private final int POST_ENTRY_TIMEOUT_MS = 30000;
    private final int FILE_THREADS = 1;

    public TuboroidAgentManager(final Context context) {
        context_ = context;
    ...
    public HttpSingleTaskAgent getSingleHttpAgent() {
        if (single_http_agent_ == null) {
            synchronized (this) {
                if (single_http_agent_ == null) {
                    final SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(context_);
                    final String cookie_data = pref.getString("pref_cookie_data", "");
                    ...
                    single_http_agent_.setSaveCookieCallback(new SaveCookieStoreCallback() {
                        @Override
                        public void saveCookieStore(final String cookieBareData) {
                            final SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(context_)
                                    .edit();
