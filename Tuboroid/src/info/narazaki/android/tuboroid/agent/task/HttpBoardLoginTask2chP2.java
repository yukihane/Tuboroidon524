...
public class HttpBoardLoginTask2chP2 {
    ...
    private final P2LoginCallback callback_;
    ...
    private class LoginFormTask extends TextHttpGetTaskBase {
        ...
        protected void dispatchHttpTextResponse(final HttpResponse res, final BufferedReader reader) throws InterruptedException,
            ...
            // hidden回収
            final Matcher matcher_hidden_form = pattern_hidden_form_.matcher(buf);
            matcher_hidden_form.reset();
            while (matcher_hidden_form.find()) {
                final String form_name = matcher_hidden_form.group(1);
                final String form_value = matcher_hidden_form.group(2);
            ...
            if (pattern_is_login_form_.matcher(buf).reset().find()) {
                executor_.submit(new Runnable() {
                    @Override
                    public void run() {
                        final LoginRequestTask next_task = new LoginRequestTask(host_name, hidden_form_map);
            ...
            executor_.submit(new Runnable() {
                @Override
                public void run() {
                    final PostFormTask next_task = new PostFormTask(host_name);
                    next_task.sendTo(http_agent_);
                }
            });
        }
    ...
    private class LoginRequestTask extends TextHttpPostTaskBase {
        private final HashMap<String, String> login_form_map_;

        public LoginRequestTask(final String host_name, final HashMap<String, String> login_form_map) {
            super("http://" + host_name + LOGIN_PATH);
        ...
        protected boolean setRequestParameters(final HttpRequestBase req) {
            try {
                ...
                if (login_form_map_ != null) {
                    for (final Entry<String, String> data : login_form_map_.entrySet()) {
                        if (buf.length() > 0)
                            buf.append("&");
        ...
        @Override
        protected void dispatchHttpTextResponse(final HttpResponse res, final BufferedReader reader)
    ...
    public HttpBoardLoginTask2chP2(final ThreadData thread_data, final AccountPref account_pref, final P2LoginCallback callback) {
    ...
    public void sendTo(final HttpTaskAgentInterface http_agent) {
        ...
        executor_.submit(new Runnable() {
            @Override
            public void run() {
                final LoginFormTask task = new LoginFormTask(P2_BASE_HOST);
