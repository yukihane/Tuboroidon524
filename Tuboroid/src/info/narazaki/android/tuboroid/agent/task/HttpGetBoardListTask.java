...
public class HttpGetBoardListTask extends TextHttpGetTaskBase {
    ...
    private final Pattern pattern_category_;
    private final Pattern pattern_board_;
    ...
    public HttpGetBoardListTask(final String request_uri, final Callback callback) {
        super(request_uri);
        callback_ = callback;
        pattern_category_ = Pattern.compile("<BR\\><B\\>([^<]+?)</B\\><BR\\>");
        pattern_board_ = Pattern.compile("^<A HREF=http://([^/]*)/([\\w\\-]+)/\\>([^<]+?)</A\\>");
    ...
    protected void dispatchHttpTextResponse(final HttpResponse res, final BufferedReader reader) throws InterruptedException,
        ...
        final List<BoardData> data_2ch_compat_list = new ArrayList<BoardData>();
        ...
        try {
            ...
            while (true) {
                ...
                if (matcher.find()) {
                    ...
                    matcher.reset();
                    if (matcher.find()) {
                        final BoardData data = BoardData.factory(order_id, matcher.group(3), current_category,
        ...
        for (final BoardData board : data_2ch_compat_list) {
            data_list.add(board);
        }
