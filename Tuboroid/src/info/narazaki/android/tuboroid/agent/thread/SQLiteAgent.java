...
public class SQLiteAgent extends SQLiteAgentBase {
    ...
    // //////////////////////////////////////////////////
    public SQLiteAgent(final Context context) {
        super(context);
    }
    ...
    private static class DBHelper extends SQLiteOpenHelper {
        ...
        private int upgradeTables(final SQLiteDatabase db, final int current_version, final int new_version, final int target_version,
            ...
            for (final String sql : alter_table_query) {
                db.execSQL(sql);
            }
    ...
    @Override
    protected SQLiteOpenHelper createSQLiteOpenHelper(final Context context) {
    ...
    public void insertBoardData(final BoardData board_data, final BoardListAgent.BoardFetchedCallback callback_created,
        ...
        submitTransaction(new DbTransaction() {
            ...
            public void run() {
                ...
                final ContentValues values = board_data.getContentValues();
                final String where = BoardData.KEY.SERVER + " = ? AND " + BoardData.KEY.TAG + " = ?";
                final String[] bind = new String[] { board_data.server_def_.board_server_, board_data.server_def_.board_tag_ };

                final Cursor cursor = getDB().query(BoardData.KEY.TABLE, BoardData.KEY.FIELD_LIST, where, bind, null, null,
                        null);
    ...
    public void getBoardList(final DbResultReceiver callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final Cursor cursor = getDB().query(BoardData.KEY.TABLE, BoardData.KEY.FIELD_LIST, null, null, null, null,
                        BoardData.KEY.PID);
    ...
    public void migrateBoardData(final BoardData to_data, final MigrateBoardCallback callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final String where = BoardData.KEY.NAME + " = ? AND " + BoardData.KEY.TAG + " = ?";
                final String[] bind = new String[] { to_data.board_name_, to_data.server_def_.board_tag_ };
                ...
                cursor.moveToFirst();
                final BoardData from_data = BoardData.factory(cursor);
                cursor.close();
                if (from_data instanceof BoardData2ch && !from_data.server_def_.equals(to_data.server_def_)) {
                    ...
                    final ContentValues new_board_values = to_data.getContentValues();
                    getDB().update(BoardData.KEY.TABLE, new_board_values, from_where, from_bind);

                    // スレ情報の書き換え
                    final ContentValues new_thread_values = new ContentValues();
    ...
    public void insertThreadData(final ThreadData thread_data, final DbTransaction callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final ContentValues values = thread_data.getContentValues();
                final String where = ThreadData.KEY.BOARD_SERVER + " = ? AND " + ThreadData.KEY.BOARD_TAG + " = ? AND "
                        + ThreadData.KEY.ID + " = ?";
                final String[] bind = new String[] { thread_data.server_def_.board_server_,
                        thread_data.server_def_.board_tag_, String.valueOf(thread_data.thread_id_) };

                final Cursor cursor = getDB().query(ThreadData.KEY.TABLE, ThreadData.KEY.FIELD_LIST, where, bind, null, null,
                        null);
                final boolean no_entry = cursor.getCount() == 0;
                cursor.close();
    ...
    public void getThreadList(final BoardData board_data, final DbResultReceiver callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final String where = ThreadData.KEY.BOARD_SERVER + " = ? AND " + ThreadData.KEY.BOARD_TAG + " = ?";
                final String[] bind = new String[] { board_data.server_def_.board_server_, board_data.server_def_.board_tag_ };
                final Cursor cursor = getDB().query(ThreadData.KEY.TABLE, ThreadData.KEY.FIELD_LIST, where, bind, null, null,
                        ThreadData.KEY.PID);
                callback.onQuery(cursor);
    ...
    public void updateThreadCacheTagData(final ThreadData thread_data, final DbTransaction callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final ContentValues values = thread_data.getCacheTagDataContentValues();
                final String where = ThreadData.KEY.BOARD_SERVER + " = ? AND " + ThreadData.KEY.BOARD_TAG + " = ? AND "
                        + ThreadData.KEY.ID + " = ?";
                final String[] bind = new String[] { thread_data.server_def_.board_server_,
                        thread_data.server_def_.board_tag_, String.valueOf(thread_data.thread_id_) };
    ...
    public void updateThreadDataList(final List<ThreadData> thread_data_list, final DbTransaction callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                for (final ThreadData thread_data : thread_data_list) {
                    final ContentValues values = thread_data.getContentValues();
                    final String where = ThreadData.KEY.BOARD_SERVER + " = ? AND " + ThreadData.KEY.BOARD_TAG + " = ? AND "
                            + ThreadData.KEY.ID + " = ?";
                    final String[] bind = new String[] { thread_data.server_def_.board_server_,
    ...
    public void getRecentList(final int recent_order, final DbResultReceiver callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final String where = ThreadData.KEY.IS_ALIVE + " = 1";
                String order = "";
                ...
                final Cursor cursor = getDB().query(ThreadData.KEY.TABLE, ThreadData.KEY.FIELD_LIST, where, null, null, null,
                        order);
                callback.onQuery(cursor);
                cursor.close();
            }
    ...
    public void updateFavoriteList(final ArrayList<BoardData> board_list, final ArrayList<ThreadData> thread_list,
        ...
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                for (final BoardData board_data : board_list) {
                    final ContentValues values = new ContentValues();
                    values.put(BoardData.KEY.IS_FAVORITE, board_data.is_favorite_ ? 1 : 0);
                    final String where = BoardData.KEY.SERVER + " = ? AND " + BoardData.KEY.TAG + " = ?";
                    final String[] bind = new String[] { board_data.server_def_.board_server_,
                            board_data.server_def_.board_tag_ };
                    getDB().update(BoardData.KEY.TABLE, values, where, bind);
                }
                for (final ThreadData thread_data : thread_list) {
                    final ContentValues values = new ContentValues();
                ...
                for (final Find2chKeyData search_key_data : search_key_list) {
                    final String where = Find2chKeyData.KEY.KEYWORD + " = ?";
                    final String[] bind = new String[] { search_key_data.keyword_ };
    ...
    public void getFavoriteBoardList(final DbResultReceiver callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final String where = BoardData.KEY.IS_FAVORITE + " = 1";
                final Cursor cursor = getDB().query(BoardData.KEY.TABLE, BoardData.KEY.FIELD_LIST, where, null, null, null,
                        null);
                callback.onQuery(cursor);
                cursor.close();
            }
    ...
    public void getFavoriteThreadList(final DbResultReceiver callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final String where = ThreadData.KEY.IS_FAVORITE + " = 1";
                final Cursor cursor = getDB().query(ThreadData.KEY.TABLE, ThreadData.KEY.FIELD_LIST, where, null, null, null,
                        null);
                callback.onQuery(cursor);
                cursor.close();
    ...
    public void getBoardOfFavoriteThreadList(final DbResultReceiver callback) {
        submitTransaction(new DbTransaction() {
            ...
            public void run() {
                ...
                final Cursor cursor = getDB().rawQuery(query, null);
                callback.onQuery(cursor);
                cursor.close();
            }
    ...
    public void getIgnoreList(final DbResultReceiver callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final Cursor cursor = getDB().query(IgnoreData.KEY.TABLE, IgnoreData.KEY.FIELD_LIST, null, null, null, null,
                        IgnoreData.KEY.PID);
                callback.onQuery(cursor);
                cursor.close();
            }
    ...
    public void addIgnoreData(final IgnoreData ignore_data) {
        submitTransaction(new DbTransactionBase() {
            @Override
            public void run() {
                final ContentValues values = ignore_data.getContentValues();
    ...
    public void deleteIgnoreData(final IgnoreData ignore_data) {
        submitTransaction(new DbTransactionBase() {
            @Override
            public void run() {
                final String where = IgnoreData.KEY.TYPE + " = ? AND " + IgnoreData.KEY.TOKEN + " = ?";
                final String[] bind = new String[] { String.valueOf(ignore_data.type_), ignore_data.token_ };
    ...
    public void getFind2chKeyList(final DbResultReceiver callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final String where = Find2chKeyData.KEY.IS_FAVORITE + " = 1";
                final Cursor cursor = getDB().query(Find2chKeyData.KEY.TABLE, Find2chKeyData.KEY.FIELD_LIST, where, null,
                        null, null, null);
                callback.onQuery(cursor);
                cursor.close();
            }
    ...
    public void getFind2chKeyData(final String keyword, final GetFind2chKeyDataResult callback) {
        submitTransaction(new DbTransaction() {
            @Override
            public void run() {
                final String where = Find2chKeyData.KEY.KEYWORD + " = ? AND " + Find2chKeyData.KEY.IS_FAVORITE + " = 1";
                final String[] bind = new String[] { keyword };
                final Cursor cursor = getDB().query(Find2chKeyData.KEY.TABLE, Find2chKeyData.KEY.FIELD_LIST, where, bind,
                        null, null, null);
    ...
    public void setFind2chKeyData(final Find2chKeyData key_data) {
        submitTransaction(new DbTransactionBase() {
            @Override
            public void run() {
                final ContentValues values = key_data.getContentValues();
                final String where = Find2chKeyData.KEY.KEYWORD + " = ?";
                final String[] bind = new String[] { key_data.keyword_ };

                final int count = getDB().update(Find2chKeyData.KEY.TABLE, values, where, bind);
