...
abstract public class BoardData implements NExpandableListAdapterDataInterface {
    ...
    public static class comparator implements Comparator<BoardData> {
        @Override
        public int compare(final BoardData object1, final BoardData object2) {
            final int a = object1.getSortOrder() - object2.getSortOrder();
            if (a != 0)
                return a;
     ...
     * Copy Constructor
     */
    protected BoardData(final BoardData boardData) {
    ...
    static public BoardData factory(final long order_id, final String board_name, final String board_category, final BoardIdentifier server_def) {
    ...
    static public BoardData factory(final Cursor cursor) {
        final long id = cursor.getLong(cursor.getColumnIndex(KEY.PID));

        final boolean is_favorite = cursor.getInt(cursor.getColumnIndex(KEY.IS_FAVORITE)) != 0;

        final boolean is_external = cursor.getInt(cursor.getColumnIndex(KEY.IS_EXTERNAL)) != 0;

        final String board_name = cursor.getString(cursor.getColumnIndex(KEY.NAME));
        final String board_server = cursor.getString(cursor.getColumnIndex(KEY.SERVER));
        final String board_tag = cursor.getString(cursor.getColumnIndex(KEY.TAG));
        final BoardIdentifier server_def = new BoardIdentifier(board_server, board_tag, 0, 0);
    ...
    static public BoardIdentifier factoryBoardServer(final Uri uri) {
    ...
    static public boolean isBoardUri(final String uri_string) {
    ...
    // コンストラクタ
    // //////////////////////////////////////////////////
    protected BoardData(final long order_id, final String board_name, final String board_category, final BoardIdentifier server_def) {
        order_id_ = order_id;
    ...
    protected BoardData(final long id, final boolean is_favorite, final boolean is_external, final String board_name, final BoardIdentifier server_def) {
        id_ = id;
    ...
    public final LinkedList<String> getLocalDatDir(final Context context) {
        final String filename = "/" + server_def_.board_server_ + "/" + server_def_.board_tag_ + "/";
    ...
    public final File getLocalSubjectFile(final Context context, final boolean use_external_storage) {
        final String filename = "/" + server_def_.board_server_ + "/" + server_def_.board_tag_ + "/subject.txt";
        File file = null;
    ...
    public void importData(final BoardData obj) {
        id_ = obj.id_;
    ...
    public boolean equals(final Object o) {
        if (o instanceof BoardData) {
            final BoardData target = (BoardData) o;
            if (isSameBoard(target))
                return true;
            return false;
        }
