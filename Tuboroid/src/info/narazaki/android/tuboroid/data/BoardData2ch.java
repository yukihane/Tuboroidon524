...
public class BoardData2ch extends BoardData {
    ...
    public static boolean is2ch(final String board_server) {
        if (board_server.indexOf(".2ch.net") != -1)
            return true;
        if (board_server.indexOf(".bbspink.com") != -1)
            return true;
        return false;
    }
    ...
    protected BoardData2ch(final BoardData2ch boardData) {
        super(boardData);
    }

    protected BoardData2ch(final long id, final boolean isFavorite, final boolean is_external, final String boardName,
            final BoardIdentifier server_def) {
        super(id, isFavorite, is_external, boardName, server_def);
    }

    protected BoardData2ch(final long orderId, final String boardName, final String boardCategory, final BoardIdentifier server_def) {
    ...
    static public BoardIdentifier createBoardIdentifierSub(final Uri uri, final String test_token, final String alt_cgi) {
        String board_server;
        String board_tag = "";
        long thread_id = 0;
        int entry_id = 0;
        try {
            ...
            if (segments.get(0).equals(test_token)
                ...
                if (segments.size() > 4 && segments.get(4).length() > 0) {
                    ...
                    // n : １を除外する → 無視して良い
                    final String opt = segments.get(4);
