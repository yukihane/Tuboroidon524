...
public class BoardDataShitaraba extends BoardData {
    ...
    public static boolean isShitaraba(final String board_server) {
        if (board_server.equals("jbbs.livedoor.jp"))
            return true;
        return false;
    }
    ...
    protected BoardDataShitaraba(final BoardDataShitaraba boardData) {
        super(boardData);
        board_category_ = CATEGORY_NAME;
    }

    protected BoardDataShitaraba(final long id, final boolean isFavorite, final boolean is_external, final String boardName,
            final BoardIdentifier server_def) {
        super(id, isFavorite, is_external, boardName, server_def);
        board_category_ = CATEGORY_NAME;
    }

    protected BoardDataShitaraba(final long orderId, final String boardName, final String boardCategory, final BoardIdentifier server_def) {
        ...
        try {
            ...
            if (segments.size() >= 4 && segments.get(0).equals("bbs")
                ...
                if (segments.size() > 5 && segments.get(5).length() > 0) {
                    ...
                    // n : １を除外する → 無視して良い
                    final String opt = segments.get(5);
                ...
                final String thread_id_seg = segments.get(3);
                if (thread_id_seg.length() > 5) {
                    final String thread_id_str = thread_id_seg.substring(0, thread_id_seg.indexOf(".html"));
