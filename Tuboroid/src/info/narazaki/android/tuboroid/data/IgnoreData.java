...
public class IgnoreData implements NListAdapterDataInterface {
    ...
    public static boolean isNG(final int type) {
        if (type != TYPE.NONE)
            return true;
        return false;
    }

    public static boolean isGone(final int type) {
        if (type == TYPE.NGID_GONE || type == TYPE.NGWORD_GONE)
            return true;
        return false;
    }
    ...
    public IgnoreData(final int type, final String token) {
        super();
        pid_ = 0;
        type_ = type;
        token_ = token;
    ...
    public IgnoreData(final Cursor cursor) {
