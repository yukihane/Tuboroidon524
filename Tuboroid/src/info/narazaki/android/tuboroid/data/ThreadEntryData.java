...
public class ThreadEntryData implements NListAdapterDataInterface {
    ...
    public static void analyzeThreadEntryList(final TuboroidAgent agent, final ThreadData thread_data,
        ...
        for (final ThreadEntryData data : data_list) {
            synchronized (thread_data) {
                ...
                // バックアンカーの抽出
                for (final long id : data.forward_anchor_list_) {
        ...
        for (final ThreadEntryData data : data_list) {
            synchronized (thread_data) {
                final ThreadEntryData orig = author_id_map.get(data.author_id_);
                if (orig != null) {
        ...
        for (final ThreadEntryData data : data_list) {
            synchronized (data) {
                final int data_img_uri_list_size = data.img_uri_list_.size();
    ...
    public static View initView(final View view, final TuboroidApplication.ViewConfig view_config,
            final ViewStyle style, final boolean is_aa) {
    ...
    public View setView(final TuboroidAgent agent, final ThreadData thread_data, final View view,
            final ViewGroup parent, final int read_count, final TuboroidApplication.ViewConfig view_config,
        ...
        final Spannable spanned = getSpannableEntryHeader(view_config, style);
        ...
        if (is_quick_show || img_uri_list_.size() == 0 || isNG()) {
            // サムネ無し
            if (tag.entry_body_view.isLongClickable() && !entry_is_aa_)
    ...
    private void initThumbnailsBox(final TableLayout thumbnail_box, final int thumbnail_cols, final int thumbnail_rows,
        ...
        final ListIterator<String> it = img_uri_list_.listIterator();
    ...
    private void setVisibilityIfChanged(final View view, final int visibility) {
    ...
    private synchronized void createSpannableEntryHeader(final TuboroidApplication.ViewConfig view_config,
        ...
        final int author_be_length = author_be_.length();
    ...
    private synchronized void createSpannableEntryBody(final TuboroidApplication.ViewConfig view_config,
        ...
        final Spannable spannable = style.spanify_.apply(body, this);
        entry_body_cache_ = new SpannableCache(spannable, style, view_config);
    }

    private synchronized Spannable getSpannableRevLink(final TuboroidApplication.ViewConfig view_config,
            final ViewStyle style) {
    ...
    private synchronized void createSpannableRevLink(final TuboroidApplication.ViewConfig view_config,
            final ViewStyle style) {
        class JumpAnchorSpan extends ClickableSpan {
            ...
            public void onClick(final View widget) {
                if (num_ > 0)
                    style_.callback_.onNumberAnchorClicked((int) entry_id_, (int) num_);
            }
        }
        ...
        synchronized (ThreadEntryData.class) {
            for (final Long num : back_anchor_list_) {
                spanned.append(" [");
