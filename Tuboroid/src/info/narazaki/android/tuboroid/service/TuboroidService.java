...
public class TuboroidService extends Service {
    ...
    private final ITuboroidService.Stub service_impl_ = new ITuboroidService.Stub() {
        ...
        private void onEndChecking() {
            if (check_update_in_background_ && check_updated_new_threads_ > 0) {
                final String message = String.valueOf(check_updated_unread_threads_) + " "
                ...
                final Notification notif = new Notification(R.drawable.stat_notify_updated, message,
                        System.currentTimeMillis());
                final Intent intent = new Intent(TuboroidService.this, NotificationReceiverActivity.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
                final PendingIntent pending = PendingIntent.getActivity(TuboroidService.this, 0, intent, 0);
            ...
            final Intent intent = new Intent(CHECK_UPDATE.ACTION_FINISHED);
        ...
        private void onCheckedUpdateBoard(final LinkedList<ThreadData> new_data_list, final Runnable on_checked_callback) {
            for (final ThreadData data : new_data_list) {
        ...
        private void sendCheckedUpdateIntent(final Runnable on_checked_callback) {
            final Intent intent = new Intent(CHECK_UPDATE.ACTION_NEW);
            intent.setPackage(getPackageName());
            ...
            executor_.submit(new Runnable() {
                ...
                public void run() {
                    getAgent().fetchFavoriteThreadList(new FavoriteThreadListFetchedCallback() {
                        @Override
                        public void onThreadListFetched(final ArrayList<ThreadData> dataList) {
            ...
            executor_.submit(new Runnable() {
                ...
                public void run() {
                    getAgent().fetchRecentList(ThreadData.KEY.RECENT_ORDER_READ, new RecentListFetchedCallback() {
                        @Override
                        public void onRecentListFetched(final ArrayList<ThreadData> dataList) {
                            beginDownloadThreadList(dataList);
                        }
                    });
                }
        ...
        private void beginDownloadThreadList(final List<ThreadData> data_list) {
            final ArrayList<ThreadData> unread_data_list = new ArrayList<ThreadData>();
            for (final ThreadData thread_data : data_list) {
        ...
        private void downloadThreadList(final List<ThreadData> data_list) {
            final ThreadData thread_data = data_list.get(0);
            data_list.remove(0);
    ...
    private WakeLock getWakeLock() {
        if (wake_lock_ == null) {
            final PowerManager pm = (PowerManager) getApplication().getSystemService(Context.POWER_SERVICE);
