...
public class TuboroidServiceTask {
    ...
    private final ServiceConnection service_conn_ = new ServiceConnection() {
        @Override
    ...
    public TuboroidServiceTask(final Context context) {
        super();
        context_ = context;
    ...
    public void bind() {
        final Intent intent = new Intent(context_, TuboroidService.class);
    ...
    private synchronized void flushServiceQueue() {
        ...
        while (sender_list_.size() > 0) {
            final ServiceSender sender = sender_list_.get(0);
