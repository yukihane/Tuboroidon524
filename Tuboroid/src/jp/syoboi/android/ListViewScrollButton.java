...
public class ListViewScrollButton extends ImageButton {
    ...
    public ListViewScrollButton(final Context context, final AttributeSet attrs, final int defStyle) {
        super(context, attrs, defStyle);
    }

    public ListViewScrollButton(final Context context, final AttributeSet attrs) {
        super(context, attrs);
    }

    public void setListView(final Activity activity, final ListView lv) {
        activity_ = activity;
        this.listView = lv;
    }
    ...
    public int[] onCreateDrawableState(final int extraSpace) {
        final int[] new_list = super.onCreateDrawableState(extraSpace);
        if (active) {
            final int[] new_list_active = new int[new_list.length + 1];
    ...
    public static void scrollBy(final ListView lv, int distance) {
        if (distance == 0 || lv.getChildCount() == 0)
            return;

        final int top = 0;
        final int bottom = lv.getMeasuredHeight();
        ...
        final View vTop = lv.getChildAt(0);
        final View vBottom = lv.getChildAt(childCount - 1);
    ...
    public static class Scroller extends CountDownTimer {
        ...
        public Scroller(final ListView listView, final long millisInFuture, final long countDownInterval) {
